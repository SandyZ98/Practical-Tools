...
## Target Population and Goal of the sample design
...
knitr::kable(age.tgt, col.names = c("Age Group","Population","n (Respondents)","Expected Response Rate","Desired Sample Size","Sampling Rate"),caption = 'Table 1 Desired Age Domain Sample Sizes')
```
...
The composite MOS for each $PSU_{ij}$, $S_i=\sum_{j\epsilon U_i}S_{ij}=\sum_df_dQ_i(d)\\$, where $S_{ij}$ is the composite MOS for $SSU_j$ in $PSU_i$ and $Q_i$ is defined as number of elements in PSU i that are in domain d. Summing the $S_i$ will give us the total composite MOS, which should be equal to the total number of sample size, 428.

Given that both PSUs and SSUs are sampled pps using the composite
MOS, the selection probability of $SSU_{ij}$ is defined as $\pi_i\pi_{k|ij}=mnS_{ij}/S$, where m is the number of sample PSUs and n is the number of sample SSUs in each PSU.Then, we are able to calculate the desired number of be selected from domain d in each SSU with $q_{ij}^*(d)=\bar{\bar{q}}f_d/S_{ij}$. It is worth to mentioning that $\bar{\bar{q}}$ is constant in each sampling stage. 
...
In the current sample frame, seven block groups listed in Table 2 were detected to be undersized. Based on the map of Prince George's County, these unqualified areas include air base, golf course, national park, and university campus. Therefore, we are confident to combine them with the nearest block group within the tract. 

The first three block groups in Table 2 are the only SSUs within the  tract. After combing them, the new block group was still have a desired sample size larger than the actual population in domain 3. We decided to keep the new group in the frame. If this new block group is sampled, we would sample more persons in domain 3 in the next sampled block group to achieve the expected sample size. Block Group 240338024082 and 240338035192 were combined with 240338024082 and 240338035191, respectively. Block Group 240338072002 and 240338072003 are university campus so there are mainly young adults living there. If we combine them together, there will still be insufficient sample for domain 2 and 3. Also, the population will be very disproportionately contributed across the domains. Therefore, we combined 240338072002 with 240338072001 and 240338072003 with 240338072004.
...
SF_BG <- read_excel("C:/Users/cd_zh/Desktop/745/SampleFrame.xlsx")
SF_Tract <- read_excel("C:/Users/cd_zh/Desktop/745/SF_Tract.xlsx")
SF_BG<-SF_BG[,1:9]
...
write.csv(samdat,"C:/Users/cd_zh/Desktop/745/SampleResult.xlsx",row.names = F)
```
